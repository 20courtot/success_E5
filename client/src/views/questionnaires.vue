<template>
  <div class="about">
    <div id="roulette">
      <h1>
        {{ this.selected.questionnaire.nom_questionnaire + ".  Temps restant " + this.timeleft}}
      </h1>
...
export default {  
        ...
        methods: { 
          timer(){
            if(this.time > 0) {
              setTimeout(() => {
                ...
                if(this.minutes < 10){
                  this.minprint = '0'+this.minutes
                }else{
                  this.minprint = this.minutes
                }
                if(this.secondes < 10){
                  this.secprint = '0'+this.secondes
                }else{
                  this.secprint = this.secondes
                }
                this.timeleft = this.minprint + ':' + this.secprint
                this.timer()
          ...
          stop(){
            console.log(this. tik + '  ' + this.selected.list_question.length)
            console.log(this.answer_content_student)
            while(this.tik != this.selected.list_question.length + 1){
              this.nextQ()
            }
          },
          get_questionnaire(){
            const requestOpt = {
              ...
              fetch("http://10.0.52.11:3000/success/getQuestionnaire", requestOpt)
        ...
        nextQ(){
          ...
          const requestOpt = {
            ...
            fetch("http://10.0.52.11:3000/success/saveAnswer", requestOpt)
                .then((response) => response.text())
                .catch((error) => { console.log("error", error); })
    ...
    created(){
      ...
      setTimeout(
        function () {
          ...
          this.time = this.selected.questionnaire.time * 60
          this.timer()
        }.bind(this),
        150
      );
