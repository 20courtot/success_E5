  ...
  <div class="manageQuestionnaire">
    ...
    <div class="m-5 d-inline-block" style="width: 500px" v-if="list_div">
      <table class="table table-stripe">
        <thead>
          <tr>
            <th colspan="5">Les questionnaires</th>
          </tr>
        </thead>
        <tbody>
          <tr>
          ...
          >
            ...
            <td>
              ...
              ><a
                type="button"
                @click="goUpdate(v._id, v.nom_questionnaire,v.time, v.matiere, v.code)"
                style="margin-left: 20px"
    ...
    <div class="m-5 d-inline-block w-25" v-if="update_div">
      ...
      <div class="btn-group">
        <div class="mb-3">
          ...
          />
          <br />
          <label for="time" class="form-label">Temps pour passer le questionnaire:</label>
          <input
            type="text"
            v-model="time_update"
            class="form-control"
...
export default {
  ...
  methods: {
    update_questionnaire() {
      ...
      fetch("http://10.0.52.11:3000/success/updateQuestionnaire", requestOpt)
    ...
    goUpdate(id, nom_questionnaire, time, matiere, code) {
    ...
    get_question(){
      if(this.id_update){
      console.log(this.time_update + 'test')
      const requestOpt = {
          method: "POST",
          body: JSON.stringify({"id_questionnaire":this.id_update}),
          headers: { 'Content-type': 'application/json; charset=UTF-8' }
        };  
        fetch("http://10.0.52.11:3000/success/getQuestionList", requestOpt)
          .then((response) => response.text())
          // Sucess
          .then((result) => {
    ...
    delete_question(id){
      ...
      fetch("http://10.0.52.11:3000/success/deleteQuestion", requestOpt)
        .then((response) => response.text())
        .catch((error) => {
          console.log("error", error);
        });
    ...
    delete_questionnaire(id) {
      ...
      fetch("http://10.0.52.11:3000/success/deleteQuestionnaire", requestOpt)
        .then((response) => response.text())
        .catch((error) => {
          console.log("error", error);
        });
      ...
      fetch("http://10.0.52.11:3000/success/deleteQuestions", requestOptb)
        .then((response) => response.text())
        .catch((error) => {
          console.log("error", error);
        });
    ...
    get_questionnaire() {
      ...
      fetch("http://10.0.52.11:3000/success/getQuestionnaireList", requestOpt)
        .then((response) => response.text())
        // Sucess
        .then((result) => {
