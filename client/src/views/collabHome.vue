...
export default {
  ...
  methods: {
    globalstat(){
     const requestOpt = { method: "POST",
      ...
      fetch("http://10.0.52.11:3000/success/getGlobalStat", requestOpt)
        ...
        headers: { "Content-type": "application/json; charset=UTF-8" }, };
      fetch("http://10.0.52.11:3000/success/getStatCollabQuest", requestOptb)
        .then((response) => response.text())
        .then((response) => {
  ...
  created() {
    console.log(Math.floor(66.66666666666667 / 1))
    this.globalstat()
